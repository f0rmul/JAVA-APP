            ...
            if (accountService.validateAccountEmail(jsonAccountTransferString.getSource_account_number(),
                ...
                if (!accountService.validateNumberExists(jsonAccountTransferString.getDestination_account_number())) {
                    return ResponseEntity.status(HttpStatus.UNPROCESSABLE_ENTITY).body(
                            new JSONObject()
                                    .put("error", "Целевой аккаунт не найден")
                                    .toString());
                }
                ...
                if (sourceAccount.getBalance().compareTo(jsonAccountTransferString.getAmount()) == -1) {
                    return ResponseEntity.status(HttpStatus.UNPROCESSABLE_ENTITY).body(
                            new JSONObject()
                                    .put("error", "Недостаточный баланс на домашнем счете")
                                    .toString());
                }
                ...
                return ResponseEntity.status(HttpStatus.UNPROCESSABLE_ENTITY).body(
                        new JSONObject()
                                .put("error", "Исходная учетная запись не найдена для данного пользователя.")
                                .toString());
            }
        } else {
            return ResponseEntity.status(HttpStatus.UNPROCESSABLE_ENTITY).body(
    ...
    public ResponseEntity<Object> balance(@Valid @RequestBody AccountBalance jsonAccountBalanceString) {
        ...
        if (accountService.validateNumberExists(jsonAccountBalanceString.getAccount_number())) {
            if (accountService.validateAccountEmail(jsonAccountBalanceString.getAccount_number(),
                ...
                return ResponseEntity.status(HttpStatus.UNPROCESSABLE_ENTITY).body(
                        new JSONObject()
                                .put("error", "Учетная запись не найдена для указанного пользователя")
                                .toString());
            }
        } else {
            return ResponseEntity.status(HttpStatus.UNPROCESSABLE_ENTITY).body(
